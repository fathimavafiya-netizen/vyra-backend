...
export class NotificationService {
  ...
  async sendSecurityAlert(data: {
    ...
    const subject = `⚠️ Sociall Security Alert: ${data.alertType.replace(/_/g, ' ')}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #0c0d14; color: #fff; border-radius: 12px; border: 1px solid #ff4a4a;">
        ...
        <p style="color: #c5c6c7; line-height: 20px;">
          We detected a critical security event on your Sociall account:
        </p>
    ...
    try {
      ...
      if (data.mobile) {
        await smsProvider.sendSms(data.mobile, `⚠️ Sociall Security Alert: ${data.alertType.replace(/_/g, ' ')}. ${data.details}`);
      }
    } catch (err: any) {
